    ...
    with open('../data/input.txt', 'r') as file:
        ...
        for a in scanners:
            for b in scanners:
                distance = 0
                for i in range(3):
                    distance += max([a.position[i], b.position[i]]) - min([a.position[i], b.position[i]])
                if distance > largest_distance:
                    print(f'Comparing {a.id} and {b.id} gave the largest number: {distance}')
...
class Scanner:
    ...
    def transform(self) -> bool:
        ...
        while len(self.chain) > 0:
            ...
            self.position = (self.position[0] + x_diff, self.position[1] + y_diff, self.position[2] + z_diff)
            self.position = changeorientation(orientation, self.position)

            base = target
        print(f'Done transforming scanner {self.id}.')
