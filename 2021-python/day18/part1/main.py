...
def main():
    with open('../data/input.txt', 'r') as file:
        ...
        snailfishnumbers: list[SnailfishNumber] = []

        for line in file:
            parsedline = parens.parseString(line.strip()).asList()
            snailfishnumbers.append(SnailfishNumber(parsedline[0], 0, None))
    
        top = snailfishnumbers[0]
        del snailfishnumbers[0]
        for i, number in enumerate(snailfishnumbers):
            print(f'Adding number {i+1}: {number.tostring()}...')
            top = addsnailfishnumbers(top, number)
            reduce(top)
        print('')
        print(f'Magnitude of final sum: {top.magnitude()}')
